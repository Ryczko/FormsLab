...
export default function QuestionTypeIcons({ type, index }: QuestionTypeIconsProps) {
  const { t } = useTranslation('common');

  return (
    <>
      <Tooltip className='z-10'  id={`my-tooltip-${index}`}  place='top' >  {`${t(`questionType.${type.toLowerCase()}`)} ${t('questionType.question')}`}</Tooltip>
      <div className="mx-1 flex h-[42px] w-[26px] items-center justify-center px-[1px] text-gray-400" data-tooltip-id={`my-tooltip-${index}`} >
